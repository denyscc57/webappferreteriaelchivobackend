...
const ControladorVenta = {
  ...
  crearVenta: async (req, res) => {
    ...
    try {
    const cajaAbierta = await new Promise((resolve, reject) => {
    ModeloCaja.obtenerUltimaCajaAbierta((err, resultados) => {
      if (err) reject(err);
      else resolve(resultados.length > 0 ? resultados[0] : null);
    });
  });
    ...
    return res.status(400).json({ 
      ...
      if (cliente_info) {
        ...
        cliente = await new Promise((resolve, reject) => {
          ModeloCliente.obtenerPorId(id_cliente, (err, resultados) => {
            if (err) reject(err);
      ...
      for (const item of items) {
        const producto = await new Promise((resolve, reject) => {
          ModeloProducto.obtenerPorId(item.id_producto, (err, resultados) => {
            if (err) reject(err);
            else resolve(resultados[0]);
          });
        });
      ...
      const numeroFactura = await new Promise((resolve, reject) => {
        ModeloVenta.generarNumeroFactura((err, factura) => {
          if (err) reject(err);
      ...
      for (const item of itemsActualizados) {
        const ventaData = {
          ...
          fecha: fecha,
          hora: hora,
          id_caja: cajaAbierta.ca_id // AÑADIR EL ID DE CAJA
        };

        console.log('Datos de venta a insertar (con caja):', ventaData);

        await new Promise((resolve, reject) => {
          ModeloVenta.agregarDetalleVenta(ventaData, (err) => {
            if (err) {
        ...
        await new Promise((resolve, reject) => {
          ModeloVenta.agregarInventario(inventarioData, (err) => {
            if (err) {
        ...
        await new Promise((resolve, reject) => {
          ModeloProducto.actualizarStock(item.id_producto, -item.cantidad, (err) => {
            if (err) {
      ...
      res.status(201).json({
        ...
        factura: numeroFactura,
        total: total,
        id_caja: cajaAbierta.ca_id, // Incluir id de caja en la respuesta
  ...
  obtenerVenta: (req, res) => {
    ...
    ModeloVenta.obtenerPorId(id, (err, resultadosVenta) => {
      if (err) {
      ...
      // Para tu estructura, necesitamos agrupar por factura
      const venta = resultadosVenta[0];
      res.json({
        venta: venta,
        items: resultadosVenta // Todos los items de esta factura
      });
    });
  },
  ...
  obtenerVentas: (req, res) => {
    ...
    ModeloVenta.obtenerTodas(filtros, (err, resultados) => {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
  ...
  obtenerEstadisticas: (req, res) => {
    ModeloVenta.obtenerEstadisticas((err, resultados) => {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
